...
../mvnw spring-boot:run -Dspring-boot.run.arguments="--spring.cloud.discovery.enabled=true --spring.cloud.service-registry.auto-registration.enabled=true --spring.cloud.consul.config.enabled=true --logging.level.sun.net.www=TRACE --spring.cloud.consul.discovery.scheme=http"
